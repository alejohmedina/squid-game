...
var timedOut = false;
var level = 1, streak = 0, lastScore = 0, runPoints = 0, runOver = false, pendingScore = 0;
var board = [];
...
function updateHud() {
  ...
  var st = $('streakTag'); if (st) st.textContent = 'RACHA ' + streak;
...
function startRun() {
  level = 1; streak = 0; runPoints = 0; pendingScore = 0; runOver = false;
  startGame();
}
...
function endGame(wasTimeout) {
  ...
  if (sc) sc.textContent = '+' + lastScore + ' pts · Total ' + runPoints + ' · Racha ' + streak;
  if (!win && bw) {
    renderBoard();
